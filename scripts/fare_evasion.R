...
bart_uof.df %>%
  ...
  write.csv("output/fare_evasion_correlated_charges.csv", row.names = FALSE)

#case distribution by race
bart_uof.df %>%
  select(`Case #`, `Citizen Race`) %>%
  filter(`Case #` %in% cases.filter) %>%
  mutate(`Citizen Race` = str_replace_all(`Citizen Race`, c("Nat American" = "Other",
                                                            "M" = "Other",
                                                            "Asian" = "Other"))) %>%
...
ggplot(aes(x = reorder(`Citizen Race`, pct_cases), y = pct_cases)) +
  ...
  labs(title = "Distribution of cases by race",
       subtitle = "Involving fare evasion charges") +
...
ggsave("output/fare_evasion_cases_by_race.png", dpi = 300, width = 5, height = 3)
...
bart_uof.df %>%
  ...
  labs(title = "Age distribution of cases involving fare evasion",
       subtitle = "",
       y = "Cases",
       x = "Citizen Age") +
...
ggsave("output/fare_evasion_age_distribution_by_race_pct.png", dpi = 300, width = 4.5, height = 3.5)



#fare evasion force tactics used
tactics.order <-
  bart_uof.df %>%
...
bart_uof.df %>%
  ...
  labs(title = "Distribution of Force Tactics by Race",
       subtitle = "As percent of cases involving fare evasion charges") +
...
bart_uof.df %>%
  ...
  geom_hline(yintercept = 0, color = "black", size = 0.25) +
  scale_y_continuous(position = "right", limits = c(0,10), breaks = scales::pretty_breaks(n = 4)) +
  coord_flip() +
...
ggsave("output/fare_evasion_cases_by_station.png", dpi = 300, width = 5, height = 6)
...
ggsave("output/fare_evasion_cases_by_hoddow.png", dpi = 300, width = 8, height = 3)
